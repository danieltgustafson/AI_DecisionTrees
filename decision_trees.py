        ...
        if sum(classes)==len(classes) or sum(classes)==0:
            ...
            if max_feature[1]==0:
                ...
                if(sum(classes)/float(len(classes))<0.5):
                    ...
                    leaf = DecisionNode(None,None,None,1)
                print sum(classes),float(len(classes))
                return(leaf)
            else:
                ...
                classes_right=classes[output==0].tolist()
                    ...
                    #return(DecisionNode(self.__build_tree__(features_left,classes_left,depth=self.depth_limit),self.__build_tree__(features_right,classes_right,depth=self.depth_limit),function))   
...
class RandomForest():
    ...
    def fit(self, features, classes):
        ...
        exam_rate=self.example_subsample_rate
        att_rate=self.attr_subsample_rate
        features=np.array(features)
        classes=np.array(classes)
        self.tree_list=[]
        self.att_list=[]
        for i in range(trees):
            j=0
            ex_list=np.array([np.random.choice((0,1),p=[1-exam_rate,exam_rate]) for j in range(len(classes))])
            self.att_list.append([np.random.choice((0,1),p=[1-att_rate,att_rate]) for j in range(len(features.T))])
            
            feature_tree=features[ex_list==1,:]
            
            class_tree = classes[ex_list==1]
            feature_tree=feature_tree.T[np.array(self.att_list[i])==1,:].T
            self.tree_list.append(DecisionTree(self.depth_limit))
            self.tree_list[i].fit(feature_tree.tolist(), class_tree.tolist())
    ...
    def classify(self, features):
        ...
        for i in range(self.num_trees):
            feature_list=features.T[np.array(self.att_list[i])==1,:].T.tolist()
            output_list.append(self.tree_list[i].classify(feature_list))
            #print max(self.tree_list[i].classify(feature_list))
        #print output_list[1:10]
        for column in np.array(output_list).T:
           
            if(sum(column)/float(len(column))>=.5):
                decision.append(1)
            else:
                decision.append(0)
        return decision
...
class ChallengeClassifier():
    ...
    def fit(self, features, classes):
        ...
        self.rf=RandomForest(3,5,.25,.75)
        self.rf.fit(features,classes)
        
    def classify(self, features):
        # classify each feature in features as either 0 or 1.
        output=self.rf.classify(features)
        return output